	...
	<div class="" id="wk-existing-delivery">
		...
		<div class="form-group {if $customer.addresses|count == 0}hidden{/if}">
			<label class="cursor-pointer text-xs align-middle">
				<input type="checkbox" name="wk-different-invoice" value="1" class="mr-2 bg-white border-2 border-gray-3000 border-solid checked:bg-main-dark checked:focus:bg-main-dark checked:hover:bg-main-dark focus:ring-0 focus:ring-transparent form-checkbox opacity-100 outline-none rounded transition" {if Configuration::get('WK_CHECKOUT_DELIVERY_AS_INVOICE')  && $cart.id_address_delivery == $cart.id_address_invoice}checked="checked"{/if}>
