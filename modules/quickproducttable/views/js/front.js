...
function fmmAddAllCart() {
  ...
  if (coun == 1) {
    ...
    $.simplyToast(
      'success',
      'Products successfully added to your shopping cart',
    )
  } else {
    ...
    $.ajax({
      ...
      success: function (data) {
        if (data == 1) {
          ...
          $.simplyToast(
            'warning',
            'Product is Not Available with this combination',
          )
        } else {
          $.simplyToast(
            'warning',
            'Product is Not Available with this Quantity',
          )
        }
      },
      error: function () {
  ...
  if (typeof search_q_val !== 'undefined' && search_q_val) {
    $.ajax({
      ...
      url:
        ...
        '&q=' +
        search_q_val +
        '&ajax=1&action=getSearchProducts&forceJson=1&disableCombination=0&exclude_packs=0&excludeVirtuals=0&limit=1&cat=' +
        cat,
      success: function (data) {
        var quicklink_list =
        ...
        $.each(data, function (index, value) {
          if (typeof data[index]['id'] !== 'undefined')
            quicklink_list +=
              '<li onclick="relSelectThis(' +
              data[index]['id'] +
              ',' +
              data[index]['id_product_attribute'] +
              ",'" +
              data[index]['name'] +
              "','" +
              data[index]['image'] +
...
function relSelectThis(id, ipa, name, img, price) {
  ...
  if ($('#row_' + id + '_' + ipa).length > 0) {
    ...
    var draw_html =
      '<li id="row_' +
      id +
      '" class="media"><div class="media-left"><img src="' +
      img +
      '" class="media-object image"></div><div class="media-body media-middle"><span class="label">' +
      name +
      '&nbsp;(PRICE:' +
      Math.round(price) +
      ')<i onclick="relDropThis(this);" class="material-icons delete">clear</i><i onclick="relAddThis(' +
      id +
      ',' +
      ipa +
      ');"class="material-icons">add_shopping_cart</i><label style="margin-left: 5%;margin-right: 10px;color: #b1aaaa;">QTY</label><input class="qty_id" id="qty_' +
      id +
      '" type="number" value="1"></span></div><input type="hidden" value="' +
      id +
      ',' +
      ipa +
      '" name="related_products[]"><input type="hidden" value="' +
      ipa +
...
$(document).ready(function () {
  ...
  var content =
    '<i class="material-icons srcicon" tabindex="0" role="button">search</i>'
